...
"""Heuristik-Achse, die beiden fehlenden Zeitmessungen.

compare_heuristics_satisficing.py hat dieselben 12 Algorithmen nur mit der
monotonen Uhr gemessen. Hier laufen dieselben vier Generatoren und dieselben
drei Heuristiken mit off und cpu, sodass sich der Messaufwand herausrechnen
laesst: SG-Zeit(gemessen) minus (Suchzeit(gemessen) minus Suchzeit(off)).

Eigener Dateiname, damit Lab ein eigenes Datenverzeichnis anlegt und der
monotone Lauf unberuehrt bleibt.
"""
...
# The two clocks missing from compare_heuristics_satisficing.py, which ran
# monotonic only. off gives the honest search time, cpu the expensive clock;
# together with the earlier monotonic run this closes the timer axis.
TIMER_MODES = [
    ("timer_off", "off"),
    ("timer_cpu", "cpu"),
]
